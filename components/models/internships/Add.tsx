...
import { InternshipDetailsSchema, SSTIFDetailsSchema } from "@/schema";
...
export const AddInternshipData = () => {
  const { isOpen, onClose, type } = useModal();
  const { token, isTokenExpired } = useSession();
  const router = useRouter();
  const isModalOpen = isOpen && type === "addInternshipData";
  ...
  const onSubmit = async (values: z.infer<typeof InternshipDetailsSchema>) => {
    try {
      ...
      await axios.post("/api/components/internships", values, {
        headers: {
